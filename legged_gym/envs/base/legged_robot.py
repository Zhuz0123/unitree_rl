...
class LeggedRobot(BaseTask):
    ...
    def compute_observations(self):
        ...
        """
        self.obs_buf = torch.cat((  self.base_lin_vel * self.obs_scales.lin_vel, #基座线速度3维
                                    self.base_ang_vel  * self.obs_scales.ang_vel, #基座角速度3维
                                    self.projected_gravity, #重力投影3维
                                    self.commands[:, :3] * self.commands_scale, #命令3维
                                    (self.dof_pos - self.default_dof_pos) * self.obs_scales.dof_pos, #关节位置12维
                                    self.dof_vel * self.obs_scales.dof_vel, #关节速度12维
                                    self.actions #上一时间步动作12维
                                    ),dim=-1)
    ...
    def save_attitude_history(self, env_id=0, path=None):
        """保存特定环境的姿态历史记录到文件
        ...
        if path is None:
            # 创建默认保存路径
            save_dir = os.path.join(LEGGED_GYM_ROOT_DIR, 'logs', 'attitude_history')
        ...
        # 添加时间索引
        time_steps = np.arange(0, history_length * self.attitude_recording_freq * self.dt, 
                              self.attitude_recording_freq * self.dt)
        time_steps = time_steps[:attitude_data.shape[0]]
        
        # 创建结构化数组便于分析
        result = np.zeros(attitude_data.shape[0], dtype=[('time', 'f4'), ('pitch', 'f4'), ('roll', 'f4')])
        result['time'] = time_steps
        result['pitch'] = attitude_data[:, 0]
        result['roll'] = attitude_data[:, 1]
